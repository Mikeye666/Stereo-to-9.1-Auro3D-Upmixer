classdef Kraft_Ambience_Extractor_original < audioPlugin
    ...
    properties (Constant)
        PluginInterface = ...
            ...
            'InputChannels',2, ...
            'OutputChannels',8, ...
            'PluginName','Kraft Ambience Extractor original (2 in / 8 out)');
    end
    methods
        %------------------------------------------------------------------
        % Construct
        %------------------------------------------------------------------
        function plugin = Kraft_Ambience_Extractor_original
            ...
            %   prepare random numbers and filter sigmas for filter
            %   initialization
            plugin.RLR = 2*rand(plugin.windowSize,1)-1;
            plugin.RS_Fr = 2*rand(plugin.windowSize,1)-1;
            sigmas = coder.load('kraftFilterDataSigma_original.dat');
